class Keyboard {
    constructor() {
        ...
        this.BracketRight = [']', 'ъ', ']', ';'];
        this.Backslash = ['\\', '\\', '|', '/'];
        this.Delete = ['Del', 'Del', 'Del', 'Del'];
        this.CapsLock = ['Caps Lock', 'Caps Lock', 'Caps Lock', 'Сaps Lock'];
        this.KeyA = ['a', 'ф', 'a', 'ф'];
        this.KeyS = ['s', 'ы', 's', 'ы'];
        this.KeyD = ['d', 'в', 'd', 'в'];
        this.KeyF = ['f', 'а', 'f', 'а'];
...
const keyProperties = new Keyboard();
...
const createPage = () => {
    ...
    description.innerHTML = '<br><i>Switch lang: left Ctrl + left Alt <br> Made in Windows<i>';
...
const shiftHandler = () => {
    const keyState = document.querySelector('.ShiftLeft').classList.contains('active')
        || document.querySelector('.ShiftRight').classList.contains('active');
    const capsLock = document.querySelector('.CapsLock');

    const noneCapsLockToUpper = () => {
        keys.forEach((element) => {
            if (!someElements.includes(element)) {
                const key = document.querySelector(`.${element}`);
    ...
    if (keyState && language === 'En') {
        switchSymbols(2);
        noneCapsLockToUpper();
    } else if (!keyState && language === 'En') {
        switchSymbols(0);
        capsLockToUpper();
    } else if (keyState && language === 'Ru') {
        switchSymbols(3);
        noneCapsLockToUpper();
    } else if (!keyState && language === 'Ru') {
        switchSymbols(1);
...
const switchLang = () => {
    const keyState = document.querySelector('.ControlLeft').classList.contains('active')
...
const textareaKeyboardListener = (event) => {
    ...
    if (keys.includes(event.code)
        && !someElements.includes(event.code)) {
        const textareaText = textarea.value.substring(0, start)
...
const textareaMouseListener = (event) => {
    ...
    if (keys.includes(event.target.classList[0])
        && !someElements.includes(event.target.classList[0])) {
        const textareaText = textarea.value.substring(0, start)
